...
Usage: {{ include "my-scala-app.collect-secrets" .Values.config }}
...
{{- $secrets := list -}}
{{- $result := include "gatling-helm-inner.collect-secrets-recursive" (dict "value" . "secrets" $secrets) | fromYaml -}}
{{- toJson $result.secrets -}}
{{- end -}}
...
{{- if kindIs "map" $value -}}
  ...
  {{- else -}}
    ...
    {{- range $key, $val := $value -}}
      {{- $result := include "gatling-helm-inner.collect-secrets-recursive" (dict "value" $val "secrets" $secrets) | fromYaml -}}
